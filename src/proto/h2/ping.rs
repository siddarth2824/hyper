...
struct Shared {
    ...
    /// Set to true each time a frame is "recorded". Set back to false
    /// when checking if a ping should be sent.
    #[cfg(feature = "runtime")]
    read_since: bool,
    ...
    pub(super) fn poll(&mut self, cx: &mut task::Context<'_>) -> Poll<Ponged> {
        ...
        #[cfg(feature = "runtime")]
        {
            if let Some(ref mut ka) = self.keep_alive {
                ka.maybe_schedule(is_idle, &mut locked);
...
#[cfg(feature = "runtime")]
impl KeepAlive {
    fn maybe_schedule(&mut self, is_idle: bool, shared: &mut Shared) {
        match self.state {
            KeepAliveState::Init => {
    ...
    fn schedule(&mut self, shared: &mut Shared) {
